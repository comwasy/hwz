...
class Index extends Base {
    ...
    public function index(){      
        ...
        $show_goods = S('show_goods');
        if(empty($show_goods)){
	        $show_goods = M('goods')
		        ->field('goods_name,goods_id,shop_price,supplier_price,cat_id,is_recommend,is_new,is_hot')
		        ->where('is_recommend = 1 and is_on_sale=1 and LOCATE(2,prefecture_ids)')
		        ->select();
	        $prefecture = M('goods_prefecture')->field('id,name,mobile_name')->where('id = 2')->find();
	        S('show_goods',$show_goods,TPSHOP_CACHE_TIME);
        }
	    $prefecture['prefecture_id'] = 2;
	    $prefecture['hot_goods'] = $show_goods;
	    $cateList[] = $prefecture;
